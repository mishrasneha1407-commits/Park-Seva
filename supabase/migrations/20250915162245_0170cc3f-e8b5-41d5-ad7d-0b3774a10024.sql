...
-- Create bookings table (simplified without the overlap constraint for now)
...
-- Insert default parking settings into admin_settings
...
-- Create some sample data for testing (only if no parking lots exist)
INSERT INTO public.parking_lots (name, address, latitude, longitude, owner_id, total_slots, hourly_rate)
SELECT 'Downtown Plaza', '123 Main St, Downtown', 40.7128, -74.0060, 
       (SELECT id FROM public.profiles WHERE role = 'admin' LIMIT 1), 50, 8.00
WHERE NOT EXISTS (SELECT 1 FROM public.parking_lots LIMIT 1);

INSERT INTO public.parking_lots (name, address, latitude, longitude, owner_id, total_slots, hourly_rate)
SELECT 'Airport Parking', '456 Airport Blvd, Terminal A', 40.6892, -74.1745,
       (SELECT id FROM public.profiles WHERE role = 'admin' LIMIT 1), 200, 12.00
WHERE (SELECT COUNT(*) FROM public.parking_lots) < 2;

INSERT INTO public.parking_lots (name, address, latitude, longitude, owner_id, total_slots, hourly_rate)
SELECT 'Shopping Center', '789 Mall Ave, West Side', 40.7282, -73.7949,
       (SELECT id FROM public.profiles WHERE role = 'admin' LIMIT 1), 100, 3.00
WHERE (SELECT COUNT(*) FROM public.parking_lots) < 3;

-- Create sample slots for the Downtown Plaza lot
INSERT INTO public.slots (lot_id, slot_number, price_per_hour, ev_supported, is_covered, is_accessible)
SELECT 
  (SELECT id FROM public.parking_lots WHERE name = 'Downtown Plaza' LIMIT 1),
...
WHERE EXISTS (SELECT 1 FROM public.parking_lots WHERE name = 'Downtown Plaza')
  AND NOT EXISTS (SELECT 1 FROM public.slots LIMIT 1);